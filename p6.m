...
var_ = var(s_k);
q_95 = mean(s_k)+1.96*sqrt(var_) % 1.7640e+03
...
for j=1:num
  ...
  s_k(j) = s_0*exp(sum(x(3:12)));
end

var_ = var(s_k);
q_05 = mean(s_k)-1.96*sqrt(var_) % -1.7703e+04
...
q_05 = mean(s_k)-1.96*sqrt(var_) % -1.1036e+03
...
q_05 = mean(s_k)-1.96*sqrt(var_) % -3.5407e+03